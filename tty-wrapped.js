...
exports.default = getTTY;

var _term = require('term.js/src/term.js');

var _term2 = _interopRequireDefault(_term);

var _socketIo = require('socket.io-client/dist/socket.io.js');
...
var EventEmitter = _term2.default.EventEmitter,
    inherits = _term2.default.inherits,
    on = _term2.default.on,
    off = _term2.default.off,
    cancel = _term2.default.cancel;
...
tty.open = function (path, username, password) {
  if (document.location.pathname) {
    ...
    tty.socket = _socketIo2.default.connect(path, { resource: resource, query: 'auth=' + btoa(btoa(username) + ':' + btoa(password)) });
  } else {
    tty.socket = _socketIo2.default.connect('http://localhost:8090/');
  }
...
function Window(socket) {
  ...
  this.cols = _term2.default.geometry[0];
  this.rows = _term2.default.geometry[1];
...
Tab.prototype.focus = function () {
  if (_term2.default.focus === this) return;
...
Tab.prototype.hookKeys = function () {
  ...
  this.on('key', function (key, ev) {
    if (_term2.default.focusKeys === false) {
      return;
    }
...
Tab.prototype.bindMouse = function () {
  if (!_term2.default.programFeatures) return this._bindMouse();
...
tty.Window = Window;
tty.Tab = Tab;
tty.Terminal = _term2.default;

var initialTTYOpen = tty.open;
tty.open = function () {};

function getTTY() {
  return [tty, initialTTYOpen];
}
